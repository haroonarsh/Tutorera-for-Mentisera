...
// TEMPORARY: send a one-off test email using the new branded template.
// Used to verify the new email chrome in real inboxes. Safe to remove.
export const sendTestEmail = async (req: AuthRequest, res: Response): Promise<void> => {
  const to = String(req.body?.to || "").trim().toLowerCase();
  const variant = String(req.body?.variant || "payment").toLowerCase();

  if (!to || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) {
    res.status(400).json({ success: false, message: "A valid `to` email is required" });
    return;
  }

  let subject = "TUTORERA® — Test Email";
  let html = "";
  ...
  if (variant === "welcome") {
  ...
  } else if (variant === "tutor_approved") {
  ...
  } else if (variant === "cnic_rejected") {
    subject = "TUTORERA® — ID verification needs attention";
    html = renderTransactionalEmail({
      subject,
      emailCategory: "Tutor Verification",
      emailHeading: "CNIC verification needs attention",
      emailSubheading: "We were unable to verify your CNIC. Please upload a clearer image.",
      firstName: "Shahid",
      openingMessage: "We were unable to verify your CNIC. Please upload a clearer image.",
      mainMessage: "Reason: Uploaded image was unclear. Please upload a clearer photo of your CNIC.",
      cta: { label: "Upload New CNIC", url: "https://tutorera.ac.pk/tutor/application-status" },
      includeSecurityNotice: true,
    });
  } else if (variant === "marketplace_activated") {
    subject = "🎉 You're live on the TUTORERA® marketplace";
    html = renderTransactionalEmail({
      subject,
      emailCategory: "Marketplace Activation",
      emailHeading: "You're live on the TUTORERA® marketplace",
      emailSubheading: "Your tutor profile is now active and visible to students.",
      firstName: "Shahid",
      openingMessage: "Congratulations! Your tutor profile is now active on the TUTORERA® marketplace.",
      mainMessage: "You may now receive tutoring opportunities and submit offers. To unlock Home and In-Person Tuition, submit your police verification certificate.",
      transaction: {
        referenceId: "TUT-2026-000184",
        date: new Date().toLocaleDateString("en-PK", { day: "2-digit", month: "short", year: "numeric" }),
        status: "Active",
        amount: "Marketplace Profile",
      },
      cta: { label: "View My Public Profile", url: "https://tutorera.ac.pk/dashboard" },
      includeSecurityNotice: true,
    });
  } else {
    ...
    html = renderTransactionalEmail({
      ...
      emailSubheading: "Your session with Ali has been confirmed.",
      firstName: "Shahid",
      openingMessage: "We've confirmed your payment of PKR 2,500 for your session with Ali (Mathematics, Matric).",
      ...
      transaction: {
        referenceId: "BID-65f0d3a1b2c4d5e6f7a8b9c0",
        date: new Date().toLocaleDateString("en-PK", { day: "2-digit", month: "short", year: "numeric" }),
        status: "Confirmed",
        amount: "PKR 2,500",
      },
      cta: { label: "View Booking", url: "https://tutorera.ac.pk/dashboard" },
      additionalInformation: "You can reschedule or cancel the booking up to 6 hours before the session.",
      includeSecurityNotice: true,
  ...
  try {
    ...
    res.status(500).json({ success: false, message: err instanceof Error ? err.message : "Failed to send" });
  }
};
