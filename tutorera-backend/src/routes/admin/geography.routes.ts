...
router.get("/", requirePermission("market.read"), async (_req: AuthRequest, res: Response) => {
  const [countries, regionCount, cityCount, localityCount] = await Promise.all([
    Country.find().sort({ enabled: -1, name: 1 }).lean(),
    Region.countDocuments(), City.countDocuments(), Locality.countDocuments(),
  ]);
  const markets = await MarketConfig.find().select("countryCode launchStatus isActive paymentsEnabled homeTuitionEnabled").lean();
...
router.patch("/countries/:id", requirePermission("market.configure"), async (req: AuthRequest, res: Response) => {
  ...
  const country = await Country.findByIdAndUpdate(req.params.id, { $set: changes }, { new: true, runValidators: true });
