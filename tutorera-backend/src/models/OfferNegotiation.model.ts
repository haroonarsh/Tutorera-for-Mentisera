...
export interface IOfferNegotiation extends Document {
  ...
  amount: number;
  currency?: string;            // ISO 4217 — the currency of this counter-offer amount
  amountUSD?: number;           // normalised USD equivalent at time of submission
  message?: string; sequenceNumber: number; expiresAt: Date;
  status: "active" | "superseded" | "accepted" | "declined" | "expired";
  flaggedForModeration: boolean;
  respondedAt?: Date;           // when the other party replied
  createdAt: Date;
}
