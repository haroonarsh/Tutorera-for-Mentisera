...
// Matches the level slugs generated by the nested /tutors/city/[slug]/[subject]/[level] route.
...
export default async function Page({ params }: Props) {
  ...
  const levelLinks = LOCAL_SUBJECT_SLUGS.slice(0, 5).includes(subjectSlug as (typeof LOCAL_SUBJECT_SLUGS)[number])
    ? TOP_LEVEL_SLUGS.map((levelSlug) => ({
        label: `${LEVELS[levelSlug]} ${subject} in ${city}`,
        href: `/tutors/city/${slug}/${subjectSlug}/${levelSlug}`,
      }))
    : [];
  ...
  return (
    <SeoTutorDirectory 
      ...
      relatedLinks={[...levelLinks, ...relatedLinks]}
    />
  );
}
