    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {[
          ...
          <button key={tab.key} onClick={() => setFilter(tab.key as typeof filter)}
            style={{ padding: '0.5rem 1.25rem', borderRadius: '999px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: '600', border: filter === tab.key ? 'none' : `1px solid ${C.border}`, backgroundColor: filter === tab.key ? C.primary : C.surface, color: filter === tab.key ? C.surface : C.gray500 }}>
            {tab.label} ({tab.count})
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ backgroundColor: C.surface, borderRadius: '0.875rem', padding: '4rem', textAlign: 'center', border: `1px solid ${C.border}` }}>
          <Shield size={40} color={C.border} style={{ margin: '0 auto 1rem' }} />
          <p style={{ color: C.gray500 }}>No claims in this category.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {filtered.map(claim => (
            <div key={claim._id} style={{ backgroundColor: C.surface, borderRadius: '0.875rem', border: `1px solid ${claim.status === "pending" ? STATUS_COLORS.warning.border : C.border}`, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '1.25rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', cursor: 'pointer' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div style={{ width: 40, height: 40, backgroundColor: STATUS_COLORS.danger.bg, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Shield size={18} color={UI_COLORS.magenta} />
                  </div>
                  <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                  <span style={{ padding: '0.2rem 0.75rem', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 700, backgroundColor: claim.status === "pending" ? STATUS_COLORS.warning.bg : claim.status === "approved" ? STATUS_COLORS.success.bg : STATUS_COLORS.danger.bg, color: claim.status === "pending" ? STATUS_COLORS.warning.color : claim.status === "approved" ? STATUS_COLORS.success.color : STATUS_COLORS.danger.color, textTransform: 'capitalize' }}>
                    {claim.status}
                  </span>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontWeight: 600, color: C.primary, fontSize: '0.875rem', margin: '0 0 2px' }}>{claim.reason}</p>
                    <p style={{ color: TEXT_COLORS.muted, fontSize: '0.75rem', margin: 0 }}>{new Date(claim.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Expanded */}
              {expanded === claim._id && (
                <div style={{ borderTop: `1px solid ${C.border}`, padding: '1.5rem', backgroundColor: C.gray50 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
                    <div style={{ backgroundColor: C.surface, borderRadius: '0.75rem', padding: '1rem', border: `1px solid ${C.border}` }}>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: TEXT_COLORS.muted, textTransform: 'uppercase', marginBottom: '0.75rem' }}>Student</p>
                    ...
                    <div style={{ backgroundColor: C.surface, borderRadius: '0.75rem', padding: '1rem', border: `1px solid ${C.border}` }}>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: TEXT_COLORS.muted, textTransform: 'uppercase', marginBottom: '0.75rem' }}>Tutor</p>
                    ...
                    <div style={{ backgroundColor: C.surface, borderRadius: '0.75rem', padding: '1rem', border: `1px solid ${C.border}` }}>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: TEXT_COLORS.muted, textTransform: 'uppercase', marginBottom: '0.75rem' }}>Session</p>
                  ...
                  {/* Claim details */}
                  <div style={{ backgroundColor: C.surface, borderRadius: '0.75rem', padding: '1rem', border: `1px solid ${C.border}`, marginBottom: '1.25rem' }}>
                    <p style={{ fontSize: '0.75rem', fontWeight: 700, color: TEXT_COLORS.muted, textTransform: 'uppercase', marginBottom: '0.5rem' }}>Claim Reason</p>
                    ...
                    <div>
                      <div style={{ marginBottom: '0.75rem' }}>
                        ...
                        <input
                          ...
                          style={{ width: '100%', padding: '0.6rem 1rem', border: `1.5px solid ${C.border}`, borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none', boxSizing: 'border-box' }}
                        />
                      </div>
                      <div style={{ display: 'flex', gap: '0.75rem' }}>
                        <button onClick={() => handleAction(claim._id, "approved")}
                          disabled={actionLoading === claim._id}
                          style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.6rem 1.25rem', backgroundColor: C.success, color: C.surface, border: 'none', borderRadius: '0.5rem', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
                          <CheckCircle size={14} /> Approve & Notify Student
                        </button>
                        <button onClick={() => handleAction(claim._id, "rejected")}
                          disabled={actionLoading === claim._id}
                          style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.6rem 1.25rem', backgroundColor: STATUS_COLORS.danger.color, color: C.surface, border: 'none', borderRadius: '0.5rem', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
                  ...
                  {claim.status !== "pending" && claim.adminNote && (
                    <div style={{ backgroundColor: C.surface, borderRadius: '0.75rem', padding: '1rem', border: `1px solid ${C.border}` }}>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: TEXT_COLORS.muted, textTransform: 'uppercase', marginBottom: '0.3rem' }}>Admin Note</p>
