...
export default function GeographyAdminPage() {
  ...
  return <div style={{ padding: "clamp(1rem,3vw,2rem)", color: TEXT_COLORS.body }}>
    <header style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "flex-start", flexWrap: "wrap", marginBottom: "1.5rem" }}>
      <div><p style={{ color: UI_COLORS.accentBright, fontWeight: 800, letterSpacing: ".08em", margin: 0, fontSize: ".78rem" }}>GLOBAL OPERATIONS</p><h1 style={{ margin: ".35rem 0", fontSize: "clamp(1.5rem,3vw,2rem)" }}>Geography registry</h1><p style={{ color: TEXT_COLORS.secondary, margin: 0 }}>GeoNames-backed geography. Enabling a country creates a discovery-only market; payment acceptance remains unavailable until a compliant provider is configured.</p></div>
      <button type="button" onClick={() => void load()} disabled={loading} style={{ display: "inline-flex", alignItems: "center", gap: ".45rem", minHeight: 44, padding: ".65rem .9rem", border: `1px solid ${UI_COLORS.border}`, borderRadius: 10, background: "white", color: UI_COLORS.accent, fontWeight: 750 }}><RefreshCw size={17} aria-hidden="true" /> Refresh</button>
    </header>
    {error && <div role="alert" style={{ border: `1px solid ${STATUS_COLORS.danger.border}`, background: STATUS_COLORS.danger.bg, color: STATUS_COLORS.danger.color, borderRadius: 10, padding: "1rem", marginBottom: "1rem" }}>{error}</div>}
    {loading && !data ? <p role="status">Loading geography registry…</p> : data && <>
      <section aria-label="Geography totals" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: ".75rem", marginBottom: "1rem" }}>{[[Globe2,"Countries",data.summary.countries],[Map,"Regions",data.summary.regions],[MapPin,"Cities",data.summary.cities],[MapPin,"Localities",data.summary.localities]].map(([Icon,label,value]) => { const Glyph = Icon as typeof Globe2; return <article key={String(label)} style={{ background: "white", border: `1px solid ${UI_COLORS.border}`, borderRadius: 12, padding: "1rem" }}><Glyph size={18} color={UI_COLORS.accentBright} aria-hidden="true" /><strong style={{ display: "block", fontSize: "1.5rem", marginTop: ".45rem" }}>{String(value)}</strong><span style={{ color: UI_COLORS.gray500 }}>{String(label)}</span></article>; })}</section>
      <section style={{ background: "white", border: `1px solid ${UI_COLORS.border}`, borderRadius: 12, overflow: "hidden" }}><div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 720 }}><thead><tr>{["Country","Currency","Languages","Launch","Payments","Market availability"].map((label) => <th key={label} scope="col" style={{ textAlign: "start", padding: ".8rem", background: UI_COLORS.gray50, color: UI_COLORS.gray600, fontSize: ".78rem" }}>{label}</th>)}</tr></thead><tbody>{data.countries.map((country) => <tr key={country._id}><th scope="row" style={cell}>{country.name} <small>({country.iso2})</small></th><td style={cell}>{country.currencyCode || "—"}</td><td style={cell}>{country.supportedLanguages?.join(", ") || "en"}</td><td style={cell}>{country.market?.launchStatus || "disabled"}</td><td style={cell}>{country.market?.paymentsEnabled ? "Enabled" : "Unavailable"}</td><td style={cell}><button type="button" aria-pressed={country.enabled} onClick={() => void toggle(country)} style={{ minHeight: 36, borderRadius: 999, border: `1px solid ${UI_COLORS.border}`, padding: ".35rem .7rem", background: country.enabled ? STATUS_COLORS.success.bg : STATUS_COLORS.neutral.bg, color: country.enabled ? STATUS_COLORS.success.color : UI_COLORS.gray500, fontWeight: 750 }}>{country.enabled ? "Enabled" : "Disabled"}</button></td></tr>)}</tbody></table></div></section>
    </>}
  </div>;
}

const cell: React.CSSProperties = { textAlign: "start", padding: ".8rem", borderTop: `1px solid ${UI_COLORS.border}`, fontSize: ".85rem" };
