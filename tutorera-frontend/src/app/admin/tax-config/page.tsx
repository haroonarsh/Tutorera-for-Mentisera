...
const TAX_TYPE_COLORS: Record<string, string> = {
  VAT: STATUS_COLORS.info.color,
  GST: STATUS_COLORS.warning.color,
...
export default function TaxConfigPage() {
  ...
  const headCell: React.CSSProperties = { ...cellStyle, fontWeight: 700, color: UI_COLORS.gray500, textTransform: "uppercase" as const, letterSpacing: "0.05em", fontSize: "0.72rem" };
    ...
    <div style={{ padding: "32px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={handleNew} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: UI_COLORS.accent, color: "white", cursor: "pointer", fontWeight: 600, fontSize: "0.875rem" }}>+ Add Country</button>
        </div>
      </div>

      {error && <div style={{ background: `${STATUS_COLORS.danger.color}22`, border: `1px solid ${STATUS_COLORS.danger.color}`, borderRadius: 8, padding: "12px 16px", marginBottom: 16, color: STATUS_COLORS.danger.border }}>{error} <button onClick={() => setError(null)} style={{ float: "right", background: "none", border: "none", color: STATUS_COLORS.danger.border, cursor: "pointer" }}>✕</button></div>}

      {loading ? (
        <div style={{ textAlign: "center", padding: "64px 0", color: UI_COLORS.gray500 }}>Loading…</div>
      ) : (
        <div style={{ background: UI_COLORS.sidebar, border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, overflow: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {configs.length === 0 ? (
                <tr><td colSpan={10} style={{ ...cellStyle, textAlign: "center", padding: "48px 16px", color: UI_COLORS.gray500 }}>No tax configurations found. Click &quot;Add Country&quot; to begin.</td></tr>
              ) : configs.map((cfg) => (
                <tr key={cfg._id} style={{ transition: "background 0.15s" }} onMouseEnter={e => (e.currentTarget.style.background = "rgba(99,102,241,0.05)")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                  ...
                  <td style={cellStyle}>{chip(cfg.taxType, TAX_TYPE_COLORS[cfg.taxType] || STATUS_COLORS.neutral.color)}</td>
                  ...
                  <td style={{ ...cellStyle, textAlign: "center" }}>{chip(cfg.isActive ? "Active" : "Inactive", cfg.isActive ? STATUS_COLORS.success.color : STATUS_COLORS.neutral.color)}</td>
                  <td style={{ ...cellStyle, textAlign: "center" }}>
                    <button onClick={() => handleEdit(cfg)} style={{ background: "none", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, padding: "4px 8px", color: UI_COLORS.gray500, cursor: "pointer" }}>✏️</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {editOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999, backdropFilter: "blur(4px)" }}>
          <div style={{ background: UI_COLORS.sidebar, borderRadius: 12, padding: 32, width: "100%", maxWidth: 520, maxHeight: "90vh", overflow: "auto", border: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <label style={{ fontSize: "0.85rem", color: UI_COLORS.gray500 }}>
                ...
                <input value={editForm.countryCode} onChange={(e) => setEditForm({ ...editForm, countryCode: e.target.value.toUpperCase() })} disabled={!isNew} maxLength={2} style={{ display: "block", width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", background: UI_COLORS.sidebar, color: UI_COLORS.gray50, marginTop: 4, textTransform: "uppercase" }} />
              </label>
              ...
              <label style={{ fontSize: "0.85rem", color: UI_COLORS.gray500 }}>
                Display Name
                <input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} placeholder="e.g. VAT (20%)" style={{ display: "block", width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", background: UI_COLORS.sidebar, color: UI_COLORS.gray50, marginTop: 4 }} />
              </label>
              ...
              <label style={{ fontSize: "0.85rem", color: UI_COLORS.gray500 }}>
                Notes
                <textarea rows={2} value={editForm.notes || ""} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} style={{ display: "block", width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", background: UI_COLORS.sidebar, color: UI_COLORS.gray50, marginTop: 4, resize: "vertical" }} />
              </label>
            </div>

            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 24 }}>
              <button onClick={() => setEditOpen(false)} style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", background: "transparent", color: UI_COLORS.gray50, cursor: "pointer", fontWeight: 600 }}>Cancel</button>
              <button onClick={handleSave} disabled={saving || !editForm.countryCode} style={{ padding: "8px 20px", borderRadius: 8, border: "none", background: UI_COLORS.accent, color: "white", cursor: "pointer", fontWeight: 600, opacity: saving || !editForm.countryCode ? 0.5 : 1 }}>{saving ? "Saving…" : "Save"}</button>
            </div>
          </div>
        </div>
      )}
