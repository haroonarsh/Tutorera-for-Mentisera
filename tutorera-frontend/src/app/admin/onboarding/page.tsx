...
import { RefreshCw, Users, BookOpen, ArrowRight, Search } from "lucide-react";
...
export default function AdminOnboardingPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (tab === "tutors") setTutors(response.data.rows || []); else setStudents(response.data.rows || []);
  ...
  return <main style={{ padding: 24, maxWidth: 1240, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 18 }}>
      <div><p style={{ margin: 0, fontSize: 12, fontWeight: 800, letterSpacing: ".12em", color: "#0329B2" }}>OPERATIONS · ONBOARDING</p><h1 style={{ margin: "6px 0", color: "#021550", fontSize: 28 }}>Application pipeline</h1><p style={{ margin: 0, color: "#52627e" }}>Track every tutor and student from registration to marketplace readiness.</p></div>
      <button type="button" onClick={load} disabled={loading} style={button}><RefreshCw size={16} /> Refresh</button>
    </header>
    <section style={card}><div style={{ borderBottom: "1px solid #e2e8f0" }}><button type="button" onClick={() => setTab("tutors")} style={tabStyle(tab === "tutors")}><BookOpen size={17}/>Tutor onboarding</button><button type="button" onClick={() => setTab("students")} style={tabStyle(tab === "students")}><Users size={17}/>Student onboarding</button></div>
      <div style={{ padding: 16, borderBottom: "1px solid #e2e8f0", background: "#fbfdff" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 14 }}>
          {Object.entries(summary).map(([key, count]) => <button type="button" key={key} onClick={() => setPhase(phase === key ? "" : key)} style={{ textAlign: "left", border: phase === key ? "2px solid #016EF8" : "1px solid #dbe5f3", background: "#fff", padding: 10, borderRadius: 10, cursor: "pointer" }}><span style={{ display: "block", color: "#52627e", fontSize: 11, fontWeight: 800 }}>{(tab === "tutors" ? tutorLabels : studentLabels)[key] || key}</span><strong style={{ fontSize: 20, color: "#021550" }}>{count}</strong></button>)}
        </div>
      ...
      {loading ? <p style={empty}>Loading application phases…</p> : tab === "tutors" ? <TutorTable rows={tutors}/> : <StudentTable rows={students}/>} 
    </section>
  </main>;
}
