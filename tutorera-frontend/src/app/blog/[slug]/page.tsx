...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main style={{ background: "#F5F7FF", minHeight: "100vh" }}>
    ...
    <article style={{ maxWidth: 800, margin: "auto", padding: "3rem 1.5rem" }}>
      ...
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 14, padding: "clamp(1.25rem,4vw,2.5rem)" }}>
        {paragraphs.map((p, i) => {
          const isHeading = p.startsWith("**") && p.endsWith("**");
          const isLast = i === paragraphs.length - 1;
          return (
            <>
              {isHeading ? <h2 key={i} style={{ color: "#021550", fontSize: "1.25rem", margin: "2rem 0 .7rem" }}>{p.replaceAll("**", "")}</h2> : <p key={i} style={{ color: "#475569", fontSize: "1.02rem", lineHeight: 1.85, marginBottom: "1rem" }}>{p}</p>}
              {i === midPoint && !isLast && (
                <AdBanner slot="7346189519" format="auto" label="Advertisement" style={{ margin: "2rem 0" }} />
              )}
            </>
          );
        })}
