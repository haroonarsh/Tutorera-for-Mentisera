...
const PLACEHOLDER_CONTENT = "Full blog content goes here...";
...
const isValidContent = (content: string): boolean => {
  ...
  if (content.trim() === PLACEHOLDER_CONTENT) return false;
...
export default function BlogPostPage() {
  ...
  useEffect(() => {
    api.get(`/blogs/${slug}`)
      .then(res => {
        const dbBlog = res.data.blog;
        // ← KEY FIX: check content quality even when API succeeds
        if (!isValidContent(dbBlog.content)) {
          const fallback = staticBlogs[slug as string];
          if (fallback) {
            // Use DB blog metadata but static content
            setBlog({ ...dbBlog, content: fallback.content });
          } else {
            setBlog(dbBlog);
          }
        } else {
    ...
    <div style={{ backgroundColor: C.gray50, minHeight: '100vh' }}>
      ...
      <div style={{ backgroundColor: C.primary, padding: '4rem 1.5rem' }}>
        <div style={{ maxWidth: '780px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', color: '#9ca3af', fontSize: '0.875rem', flexWrap: 'wrap' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <User size={14} />{blog.author?.name || "TUTORERA Team"}
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Calendar size={14} />
              {new Date(blog.createdAt).toLocaleDateString("en-PK", { month: "long", day: "numeric", year: "numeric" })}
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '780px', margin: '0 auto', padding: '3rem 1.5rem' }}>
        <div style={{ backgroundColor: 'white', borderRadius: '0.875rem', padding: '2.5rem', border: '1px solid #e5e7eb' }}>
          {blog.content.split('\n\n').map((para, i) => {
            ...
            if (trimmed.startsWith('**') && trimmed.endsWith('**')) {
              return (
                <h3 key={i} style={{ fontSize: '1.15rem', fontWeight: '700', color: C.primary, margin: '1.75rem 0 0.75rem' }}>
                  {trimmed.replace(/\*\*/g, '')}
                </h3>
              );
            }
            ...
            if (trimmed.includes('**')) {
              ...
              return (
                <p key={i} style={{ color: C.gray500, lineHeight: '1.8', marginBottom: '1rem', fontSize: '0.975rem' }}>
                  {parts.map((part, j) =>
                    j % 2 === 1
                      ? <strong key={j} style={{ color: C.primary }}>{part}</strong>
                      : part
                  )}
                </p>
              );
        ...
        <div style={{ backgroundColor: C.primary, borderRadius: '0.875rem', padding: '2rem', marginTop: '2rem', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/tutors" style={{ backgroundColor: C.accent, color: 'white', padding: '0.75rem 1.75rem', borderRadius: '0.5rem', fontWeight: '700', textDecoration: 'none', fontSize: '0.9rem' }}>
              Find a Tutor
            </Link>
            <Link href="/register" style={{ border: '1.5px solid white', color: 'white', padding: '0.75rem 1.75rem', borderRadius: '0.5rem', fontWeight: '600', textDecoration: 'none', fontSize: '0.9rem' }}>
              Sign Up Free
            </Link>
          </div>
        </div>
