...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main style={{ background: "#f9fafb", minHeight: "100vh" }}>
    <header style={{ background: "#1a1a2e", padding: "4rem 1.5rem" }}><div style={{ maxWidth: 800, margin: "auto" }}>
    ...
    <article style={{ maxWidth: 800, margin: "auto", padding: "3rem 1.5rem" }}>
      <aside style={{ background: "#eff6ff", border: "1px solid #bfdbfe", padding: "1rem", borderRadius: 10, color: "#1e3a8a", marginBottom: 24 }}><p style={{ display: "flex", gap: 7, alignItems: "center", fontWeight: 700 }}><ShieldCheck size={17}/>Editorial accountability</p><p style={{ marginTop: 5, lineHeight: 1.6 }}>Written by <Link href={blog.author.url}>{blog.author.name}</Link>. Reviewed for platform-process accuracy by <Link href={blog.reviewer.url}>{blog.reviewer.name}</Link>. General educational guidance; academic outcomes are not guaranteed.</p></aside>
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 14, padding: "clamp(1.25rem,4vw,2.5rem)" }}>{blog.content.split("\n\n").map((p, i) => p.startsWith("**") && p.endsWith("**") ? <h2 key={i} style={{ color: "#1a1a2e", fontSize: "1.25rem", margin: "2rem 0 .7rem" }}>{p.replaceAll("**", "")}</h2> : <p key={i} style={{ color: "#475569", fontSize: "1.02rem", lineHeight: 1.85, marginBottom: "1rem" }}>{p}</p>)}</div>
    </article>
  </main>;
}
