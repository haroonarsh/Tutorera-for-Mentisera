...
export default async function BlogPage() {
  ...
  return <main style={{ background: "#f9fafb", minHeight: "100vh" }}>
    <header style={{ background: "#1a1a2e", padding: "4rem 1.5rem", textAlign: "center" }}>
    ...
    <section style={{ maxWidth: 1100, margin: "auto", padding: "3rem 1.5rem" }} aria-label="Published guides">
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: "1.75rem" }}>
        {blogs.map(blog => <article key={blog._id} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 14, padding: "1.5rem", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>{blog.tags.slice(0, 3).map(tag => <span key={tag} style={{ background: "#eff6ff", color: "#2563eb", padding: "3px 8px", borderRadius: 20, fontSize: 12 }}>{tag}</span>)}</div>
          <h2 style={{ color: "#1a1a2e", fontSize: "1.15rem", lineHeight: 1.4, marginBottom: 10 }}><Link href={`/blog/${blog.slug}`} style={{ color: "inherit", textDecoration: "none" }}>{blog.title}</Link></h2>
          ...
          <Link href={`/blog/${blog.slug}`} style={{ color: "#2563eb", fontWeight: 700, textDecoration: "none", display: "flex", gap: 5, alignItems: "center", marginTop: 15 }}>Read guide <ArrowRight size={15}/></Link>
        </article>)}
      </div>
    </section>
  </main>;
