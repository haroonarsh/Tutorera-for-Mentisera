...
| Recurring bookings/packages missing | `recurring.routes.ts`, `RecurringPlan`, `RecurringBooking`, `/recurring` | Implemented end-to-end foundation; payment settlement and tutor schedule reconciliation remain |
...
4. Tightened recurring route authorization: student/parent-only plan management, tutor-only session completion/statistics, and admin-only tutor-specific statistics.
5. Added `/recurring` with accessible plan cards, role-aware bookings, progress tracking, pause/resume/cancel controls, and tutor session completion.
6. Added a recurring-learning entry point to tutor profiles and authenticated dashboard navigation.
7. Added `/research/tutoring-index`, privacy-safe methodology copy, Dataset JSON-LD, sitemap inclusion, footer discovery, and LLM documentation.
...
- Recurring subscription records currently create an active plan before a recurring payment authorization/ledger entry is confirmed. Production recurring billing must be coupled to an idempotent payment mandate before charging or promising renewal.
- Recurring plans do not yet generate concrete `Booking` records for every future session; they track package usage only. Scheduling, cancellation policy, tutor availability conflicts, and per-session fee snapshots require completion.
...
- Recurring plans now have a management page, but parent child selection and recurring checkout from a selected booking are not yet connected.
...
1. Recurring subscription creation must not leave an active unpaid plan when payment authorization fails.
2. Tutor session completion must be idempotent and must not increment relationship counts twice.
3. Parent-created checkout must preserve the child student, parent payer, and fee snapshot across webhook retries.
4. Liquidity and Tutoring Index aggregates must never expose exact addresses, contacts, emails, or individual request narratives.
5. All role-protected routes must be tested with student, parent, tutor, admin, suspended, expired-token, and unauthenticated fixtures.
6. Cloudflare Worker production smoke tests must cover `/api/v1`, `/recurring`, `/tutoring-index`, auth redirects, and canonical/noindex behavior for any legacy Vercel host.

## Prioritised build order

1. Couple recurring plans to payment mandates and concrete Booking creation.
...
6. Consolidate the remaining frontend design-system debt and migrate middleware to proxy.

The repository is therefore not “fully complete”: the marketplace foundation and most P0/P1 surfaces are present, while the listed P2 intelligence, recurring-payment integrity, parent consent, provider settlement, and operational observability items remain genuine work.
